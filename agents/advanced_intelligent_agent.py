"""advanced_intelligent_agent.py — Agente Inteligente Mejorado (con esteroides)
Análisis completo y respuestas conversacionales.

Mejora sobre la versión original: logging en vez de print() suelto dentro
de comparar_empresas_conversacional (ese print imprimía a stdout en cada
request del endpoint /comparar-conversacional, ensuciando los logs del
servidor).
"""
...
class AdvancedIntelligentAgent:
    ...
    def analizar_empresa_conversacional(self, ticker: str) -> str:
        """
        ...
        pred_corto = self.prediction_engine.predecir_corto_plazo(
            ticker, recomendacion_base['score_final'], catalistas
        )
        pred_mediano = self.prediction_engine.predecir_mediano_plazo(
            ticker, recomendacion_base['score_fundamental'], tendencia_noticias['score'] * 100
        )
        pred_largo = self.prediction_engine.predecir_largo_plazo(
            ticker, recomendacion_base['score_fundamental'], 70  # sector outlook default
        )
        escenarios = self.prediction_engine.generar_escenarios(
            ticker, precio_actual, recomendacion_base['score_fundamental']
        )
        ...
        return self.response_generator.generar_analisis_conversacional(
            ticker, empresa, datos_completos
        )

    def comparar_empresas_conversacional(self, tickers: list[str]) -> str:
        """Compara empresas de forma conversacional."""
        logger.info("Analizando %d empresas...", len(tickers))
...
"""
    ...
    def obtener_recomendacion_rapida(self, ticker: str) -> str:
